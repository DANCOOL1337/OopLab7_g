...
namespace lab_2_2
{
    internal class Structures
    {
        ...
        public static LinkedList<int> Task7(LinkedList<int> oldList)
        {
            ...
            while (curr != null)
            {
                if (curr.Value < minNode.Value) minNode = curr;
                if (curr.Value > maxNode.Value) maxNode = curr;
                curr = curr.Next;
            }

            
            Console.WriteLine("Min: " + minNode.Value);
            Console.WriteLine("Max: " + maxNode.Value);

            
            LinkedListNode<int> firstBoundary = null;
            LinkedListNode<int> secondBoundary = null;

            curr = list.First;
            while (curr != null)
            {
                if (curr == minNode || curr == maxNode)
                {
                    if (firstBoundary == null) firstBoundary = curr;
                    else { secondBoundary = curr; break; }
                }
                curr = curr.Next;
            }

            
            if (firstBoundary == secondBoundary || firstBoundary.Next == secondBoundary)
            {
                return list;
            }

            
            LinkedListNode<int> left = firstBoundary.Next;
            LinkedListNode<int> right = secondBoundary.Previous;
